#!/bin/bash
# æ­¤èæ¬å¨Imagebuilder æ ¹ç®å½è¿è¡?
...
if [ -z "$CUSTOM_PACKAGES" ]; then
  echo "âªï¸ æªéæ© ä»»ä½ç¬¬ä¸æ¹è½¯ä»¶å
"
else
  # ============= åæ­¥ç¬¬ä¸æ¹æä»¶åº==============
  # åæ­¥ç¬¬ä¸æ¹è½¯ä»¶ä»åºrun/ipk
  echo "ð æ­£å¨åæ­¥ç¬¬ä¸æ¹è½¯ä»¶ä»åº?Cloning run file repo..."
  git clone --depth=1 https://github.com/wukongdaily/store.git /tmp/store-run-repo

  # æ·è´ run/arm64 ä¸ææ?run æä»¶åipkæä»¶ å?extra-packages ç®å½
  mkdir -p extra-packages
  cp -r /tmp/store-run-repo/run/arm64/* extra-packages/

  echo "â?Run files copied to extra-packages:"
  ls -lh extra-packages/*.run
  # è§£åå¹¶æ·è´ipkå°packagesç®å½
...
# ============= iStoreOS 24.10 å®æ¹éææä»¶===================
# ============= è¥å¯ç?åæå¼æ³¨é ============================

# å®ä¹åå§ååé?
PACKAGES=""

# iStoreOSå®æ¹éæåè¡¨ï¼è¥å¯ç¨åæå¼æ³¨é
...
# N1æ çº¿ï¼æ­¤åºä»¶æªèèæ çº¿ï¼éèªè¡ç ç©¶
#PACKAGES="$PACKAGES kmod-brcmfmac wpad-basic-mbedtls"

# file/packagesç®å½çç¬¬ä¸æ¹å¯éæä»¶ï¼éè¦åå»ææ³¨éå³å¯
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - æå»ºæå."